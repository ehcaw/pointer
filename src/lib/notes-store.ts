...
interface NotesStore {
  ...
  createNewNote: (name: string, parentId: ObjectId | null, path: ObjectId[]) => Node;
...
export const useNotesStore = create<NotesStore>((set, get) => ({
  ...
  addOpenUserNote: (note: Node) => {
    ...
    const state = get();
    if (!state.openUserNotes.some(n => n._id.toString() === note._id.toString())) {
      set({ 
  ...
  closeUserNote: (noteId: string) => {
    ...
    const isUnsaved = state.unsavedNotes.has(noteId) || 
    ...
    const currentNote = openUserNotes.length > 0 ? openUserNotes[openUserNotes.length - 1] : null;
  ...
  markNoteAsUnsaved: (note: Node) => {
    ...
    const index = openUserNotes.findIndex(n => n._id.toString() === note._id.toString());
  ...
  createNewNote: (name: string, parentId: ObjectId | null, path: ObjectId[]) => {
    const state = get();
    ...
    const newNote: FileNode = {
      ...
      tenantId: state.userNotes.length > 0 
  ...
  discardAllChanges: () => {
    ...
    const openUserNotes = state.openUserNotes.map(note => {
      if (state.unsavedNotes.has(note._id.toString())) {
        // Find original note
        return state.userNotes.find(n => n._id.toString() === note._id.toString()) || note;
      }
      return note;
    }).filter(note => {
  ...
  discardChanges: (noteId: string) => {
    ...
    const isNewNote = state.newUnsavedNotes.some(n => n._id.toString() === noteId);
    ...
    if (isNewNote) {
      ...
      let currentNote = state.currentNote;
      if (currentNote && currentNote._id.toString() === noteId) {
        currentNote = openUserNotes.find(n => n._id.toString() === noteId) || null;
      }
    ...
    try {
      ...
      const openUserNotes = state.openUserNotes.map(openNote => {
        const savedVersion = savedNotes.find(
          n => n._id.toString() === openNote._id.toString() || 
      ...
      if (currentNote) {
        const savedVersion = savedNotes.find(
          n => n._id.toString() === currentNote?._id.toString() || 
              (currentNote._id.toString().startsWith("temp-") && 
      ...
      savedNotes.forEach(savedNote => {
        if (!savedNote.parentId && !treeStructure.some(n => n._id.toString() === savedNote._id.toString())) {
          treeStructure.push(savedNote);
        }
      });
