...
import { SlashCommandItem, searchSlashCommands } from '@/components/tiptap-ui/slash-command-registry'
...
const SlashCommandContent: React.FC<{ editor: Editor; onClose: () => void; query?: string }> = ({ editor, onClose, query = '' }) => {
  ...
  const handleSelectItem = useCallback((item: SlashCommandItem) => {
    const { state } = editor
  ...
  useEffect(() => {
    ...
    const container = popupRef.current?.querySelector('.slash-command-items') as HTMLElement
    ...
    if (selectedElement && container) {
      ...
      if (elementRect.top < containerRect.top) {
        ...
        // Scroll down
        container.scrollTop = selectedElement.offsetTop + (selectedElement as HTMLElement).offsetHeight - container.offsetHeight + 8
      }
    }
  }, [selectedIndex])
  ...
  const getIcon = (iconName: string) => {
    ...
    switch (iconName) {
      case 'h1': return <HeadingOneIcon {...iconProps} />
      case 'h2': return <HeadingTwoIcon {...iconProps} />
      case 'h3': return <HeadingThreeIcon {...iconProps} />
      case 'bold': return <BoldIcon {...iconProps} />
      case 'italic': return <ItalicIcon {...iconProps} />
      case 'strike': return <StrikeIcon {...iconProps} />
      case 'code': return <Code2Icon {...iconProps} />
      case 'underline': return <UnderlineIcon {...iconProps} />
      case 'bulletList': return <ListIcon {...iconProps} />
      case 'orderedList': return <ListOrderedIcon {...iconProps} />
      case 'taskList': return <ListTodoIcon {...iconProps} />
      case 'image': return <ImagePlusIcon {...iconProps} />
      case 'link': return <LinkIcon {...iconProps} />
      case 'blockquote': return <BlockQuoteIcon {...iconProps} />
      case 'codeBlock': return <CodeBlockIcon {...iconProps} />
      default: return null
    }
  }
  ...
  return (
    ...
    >
      ...
      <div className="slash-command-items" style={{ maxHeight: '280px', overflowY: 'auto' }}>
        {filteredItems.length === 0 ? (
          <div className="slash-command-no-results" style={{ padding: '16px', textAlign: 'center', color: '#6b7280', fontSize: '14px' }}>
            {query ? `No commands found for "${query}"` : 'Type to search commands...'}
          </div>
        ) : (
          filteredItems.map((item, index) => (
            <div
              ...
              style={{
                ...
                backgroundColor: index === selectedIndex ? 'rgba(59, 130, 246, 0.08)' : 'transparent',
                borderLeft: index === selectedIndex ? '3px solid #3b82f6' : '3px solid transparent',
                color: index === selectedIndex ? '#111827' : '#4b5563',
              }}
            >
              <div className="slash-command-item-icon" style={{ marginRight: '12px', opacity: 0.8, flexShrink: 0 }}>
                {getIcon(item.icon || '')}
              </div>
              <div className="slash-command-item-content" style={{ flex: 1, minWidth: 0 }}>
                <div className="slash-command-item-title" style={{ fontSize: '14px', fontWeight: 500, color: '#111827', marginBottom: '2px' }}>
                  {item.title}
                </div>
                <div className="slash-command-item-description" style={{ fontSize: '12px', color: index === selectedIndex ? '#4b5563' : '#6b7280', lineHeight: '1.4' }}>
                  {item.description}
                </div>
              </div>
            </div>
...
export const SlashCommandPopup: React.FC<SlashCommandPopupProps> = ({ editor, onClose, query }) => {
  if (!editor) {
    return null
  }

  return <SlashCommandContent editor={editor} onClose={onClose} query={query} />
}