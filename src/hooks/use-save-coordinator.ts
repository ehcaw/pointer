...
export function useSaveCoordinator() {
  ...
  const lastSaveContentRef = useRef<
    Map<string, { title?: string; content?: string; tiptapHash?: number }>
  >(new Map());
  ...
  const mergeRequests = useCallback(
    (requests: SaveRequest[]): SaveRequest[] => {
          ...
          if (!existing || request.timestamp > existing.timestamp) {
            ...
            if (existing) {
              request.changes = {
                ...
                lastEdited:
                  request.changes.lastEdited || existing.changes.lastEdited,
              };
   ...
   */
  const processSaveRequest = useCallback(
    async (request: SaveRequest): Promise<boolean> => {
      try {
        ...
        if (request.version < currentVersion) {
          console.warn(
            `Save request version ${request.version} is outdated for note ${request.noteId}, current version: ${currentVersion}`,
          );
          customInfoToast(
            "Note was updated by another save, changes were merged",
          );
        ...
        if (request.changes.content && isFile(updatedNote)) {
          ...
          if (request.changes.content.tiptap !== undefined) {
            updatedNote.content.tiptap = ensureJSONString(
              request.changes.content.tiptap,
            );
          }
        ...
        (
          updatedNote as Node & { _version?: number; _saveTimestamp?: string }
        ...
        if (
          request.version >= (noteVersionsRef.current.get(request.noteId) || 0)
        ) {
        ...
        // Update last saved content tracking AFTER save attempt (success or failure)
        // Only update fields that were actually changed in this request
        const lastContent = lastSaveContentRef.current.get(request.noteId) || {
          title: undefined,
          content: undefined,
          tiptapHash: undefined,
        };

        const updatedTracking = { ...lastContent };

        if (request.changes.title !== undefined) {
          updatedTracking.title = request.changes.title;
        }

        if (request.changes.content) {
          if (request.changes.content.text !== undefined) {
            updatedTracking.content = request.changes.content.text;
          }
          if (request.changes.content.tiptap !== undefined) {
            updatedTracking.tiptapHash = toHash(request.changes.content.tiptap);
          }
        }

        lastSaveContentRef.current.set(request.noteId, updatedTracking);
      }
    },
    ...
    try {
      ...
      for (const request of mergedRequests) {
        ...
        while (!success && attempts < maxAttempts) {
          try {
            ...
            const currentAttempts =
              retryAttemptsRef.current.get(request.noteId) || 0;
            retryAttemptsRef.current.set(request.noteId, currentAttempts + 1);

            console.error(
              `Save attempt ${attempts} failed for note ${request.noteId}:`,
              error,
            );
        ...
        if (!success) {
          ...
          customErrorToast(
            `Failed to save note after ${maxAttempts} attempts. Please check your connection.`,
          );
          console.error(
            `Max retry attempts reached for note ${request.noteId}`,
          );
        }
  ...
  const hasContentChanged = useCallback(
    (noteId: string, changes: SaveRequest["changes"]): boolean => {
      const lastContent = lastSaveContentRef.current.get(noteId);

      if (!lastContent) return true;

      if (changes.title !== undefined && changes.title !== lastContent.title)
        return true;

      if (changes.content) {
        const currentContent = changes.content.text || "";
        const currentTiptap = changes.content.tiptap || "";
        if (currentContent !== lastContent.content) return true;
        if (toHash(currentTiptap) !== lastContent.tiptapHash) return true;
      }
  ...
  const queueSave = useCallback(
    (
      ...
      return new Promise((resolve, reject) => {
        // Check if content actually changed, but bypass check for immediate saves
        if (debounce && !hasContentChanged(noteId, changes)) {
          resolve(true);
          return;
        }
        ...
        const existingIndex = requestQueueRef.current.findIndex(
          (req) => req.noteId === noteId,
        );
        if (existingIndex >= 0) {
  ...
  const saveTitle = useCallback(
    (noteId: string, title: string): Promise<boolean> => {
      // Mark as unsaved using the latest snapshot and apply title change
      const snapshot = captureNoteSnapshot(noteId);
      if (snapshot && snapshot.name !== title) {
        // Create updated note with all pending changes from the queue
        const pendingRequest = requestQueueRef.current.find(
          (req) => req.noteId === noteId,
        );
        ...
        const updatedNote = {
          ...snapshot,
          name: title,
          ...(pendingRequest?.changes.content && isFile(snapshot)
            ? {
                content: {
                  ...snapshot.content,
                  ...pendingRequest.changes.content,
                },
              }
            : {}),
        };
      ...
      if (snapshot && isFile(snapshot)) {
        ...
        if (currentText !== content.text || currentTiptap !== content.tiptap) {
          ...
          const updatedNote = {
            ...
            ...(pendingRequest?.changes.title !== undefined
              ? { name: pendingRequest.changes.title }
              : {}),
          };
  ...
  /**
   * Force immediate save (no debouncing, bypasses change detection)
   */
  const forceSave = useCallback(
    async (noteId?: string): Promise<boolean> => {
      const targetNoteId = noteId || currentNote?.pointer_id;
      if (!targetNoteId) return false;

      // Clear any existing debounce timer for this note
      const existingTimer = debounceTimersRef.current.get(targetNoteId);
      if (existingTimer) {
        clearTimeout(existingTimer);
        debounceTimersRef.current.delete(targetNoteId);
      }

      const snapshot = captureNoteSnapshot(targetNoteId);
      if (snapshot) {
        // Force save with empty changes but debounce=false to bypass change detection
        return queueSave(targetNoteId, {}, false);
      }
      return false;
    },
  ...
  const getSaveStatus = useCallback(
    (noteId: string) => {
      const hasPendingRequest = requestQueueRef.current.some(
        (req) => req.noteId === noteId,
      );
      const isCurrentlySaving =
        coordinatorState.currentRequestId?.startsWith(noteId);

      return {
        isSaving: isCurrentlySaving || coordinatorState.isProcessing,
        hasPendingSave: hasPendingRequest,
        pendingRequestCount: requestQueueRef.current.filter(
          (req) => req.noteId === noteId,
        ).length,
      };
    },
  ...
  useEffect(() => {
    ...
    const cleanupInterval = setInterval(() => {
      ...
      const activeNoteIds = new Set([
        noteId,
        ...Array.from(noteSnapshotsRef.current.keys()),
      ]);
